...
if __name__ == '__main__':
    ...
    try:
        ...
        # 发生异常
        db.rollback()
        print('插入表错误，或者已经插入字段')
    db.close()

    db = pymysql.connect('localhost', user='root', password='mysql', db='test')
    cursor = db.cursor()
    sql = 'ALTER TABLE weibo ADD COLUMN sentiment VARCHAR(100) not Null'
    try:
        cursor.execute(sql)
    except:
        # 发生异常
    ...
    for row in results:
        ...
        print(n_icd)
        try:
            sql="INSERT INTO weibo (seg_word,sentiment) VALUES(%s,%s)"%( " ".join(snow_nlp.seg_word(n_icd)),snow_nlp.setiment_score(n_icd))
            print(" ".join(snow_nlp.seg_word(n_icd)))
            print(snow_nlp.setiment_score(n_icd))
            cursor.execute(sql)
        except:
            print("读取数据出现错误")
